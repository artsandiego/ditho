import { context2d, createCanvas } from "./canvas"

/** Roughly the longest edge we aim for in an exported PNG. */
const EXPORT_TARGET_EDGE = 2048

/**
 * Upscale by a whole number with smoothing off, so exported pixels stay square
 * and the file matches what is on screen instead of a blurred version of it.
 */
export function toExportCanvas(image: ImageData): HTMLCanvasElement {
  const base = createCanvas(image.width, image.height)
  context2d(base).putImageData(image, 0, 0)

  const scale = Math.max(
    1,
    Math.floor(EXPORT_TARGET_EDGE / Math.max(image.width, image.height)),
  )
  if (scale === 1) return base

  const out = createCanvas(image.width * scale, image.height * scale)
  const ctx = context2d(out)
  ctx.imageSmoothingEnabled = false
  ctx.drawImage(base, 0, 0, out.width, out.height)
  return out
}

export function exportFilename(sourceName: string): string {
  const stem = sourceName.replace(/\.[^./]+$/, "") || "image"
  return `${stem}-dithered.png`
}

export async function downloadPng(image: ImageData, filename: string): Promise<void> {
  const canvas = toExportCanvas(image)
  const blob = await new Promise<Blob | null>((resolve) =>
    canvas.toBlob(resolve, "image/png"),
  )
  if (!blob) throw new Error("Could not encode the PNG.")

  const url = URL.createObjectURL(blob)
  const anchor = document.createElement("a")
  anchor.href = url
  anchor.download = filename
  anchor.rel = "noopener"

  // Firefox ignores clicks on detached anchors, and revoking the URL in the
  // same tick can cancel the download before the browser has read the blob.
  document.body.append(anchor)
  anchor.click()
  anchor.remove()
  setTimeout(() => URL.revokeObjectURL(url), 30_000)
}
